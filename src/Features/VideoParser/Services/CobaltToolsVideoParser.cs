...
) : IVideoParser
{
    ...
    public async Task<Result<IAlbumInputMedia[]>> GetInputFiles(string url)
    {
        ...
        try
        {
            ...
            var content = await response.Content.ReadFromJsonAsync<CobaltToolsResponse>().ConfigureAwait(false);
            ...
            switch (content.Status)
            {
                ...
                case Status.Redirect when content.Url is not null:
                    var video = await DownloadVideoAsync(content.Url, content.Filename).ConfigureAwait(false);
                    return new[] { new InputMediaVideo(video) };

                case Status.Picker when content.Picker is not null:
                    return await DownloadAllAsync(content.Picker).ConfigureAwait(false);
    ...
    private async Task<IAlbumInputMedia[]> DownloadAllAsync(PickerObject[] pickerObjects) => 
        await Task.WhenAll(pickerObjects.Select(DownloadSingleMedia)).ConfigureAwait(false);

    private async Task<InputFileStream> DownloadVideoAsync(string requestUri, string? filename = null)
    {
    ...
    private async Task<IAlbumInputMedia> DownloadSingleMedia(PickerObject picker)
    {
        var response = await client.GetAsync(picker.Url).ConfigureAwait(false);
        var stream = await response.Content.ReadAsStreamAsync().ConfigureAwait(false);
