...
{
    public sealed class Handler(IOutgoingTelegramBot bot, ICommandResolver resolver, IBotLocalizer loc) : IRequestHandler<HelpCommand, Message>
    {
        public async Task<Message> Handle(HelpCommand request, CancellationToken cancellationToken)
        {
            var help = resolver.GetCommandsByCulture(CultureInfo.CurrentUICulture)
                .Aggregate(
                    new StringBuilder(loc[Loc.HelpHeader]).AppendLine(),
                    (b, c) => b.AppendLine($"\u26a1\ufe0f `{c.Command}` - {c.Description}"),
                    sb => sb.AppendLine().Append(loc[Loc.HelpFooter]).ToString()
                );

            return await bot.SendReplyMessage(request.Message, help, cancellationToken: cancellationToken).ConfigureAwait(false);
        }
    }
