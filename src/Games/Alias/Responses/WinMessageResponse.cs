...
namespace Himawari.Alias.Responses;

public record WinMessageResponse(Message Message) : IResponse
{
    public sealed class Handler(Bot bot, IAliasService service) : IRequestHandler<WinMessageResponse, Message>
    {
        public async Task<Message> Handle(WinMessageResponse request, CancellationToken cancellationToken)
        {
